...
const ListingExcerpt = ({id, data, image}) => {
    ...
    return (
        <article className="listing-excerpt">
            <Link to={`/workshop4-interactions-mechanics/listing/${id}`}>
            ...
            <div>
                ...
                <div>
                    <p>{excerpt}</p>
                    <p><Link to={`/workshop4-interactions-mechanics/listing/${id}`} aria-label={`Read more about ${listingName}`}>Read more</Link></p>
                </div>
            </div>
        </article>    
    )
